...
function createRule(description, errorMessage, isViolation) {
  return {
    ...
    create: function (context) {
      ...
      const { nativeAllowList, allowComponents } = options[0] || {};
      return {
        JSXAttribute: function (node) {
          ...
          if (
            Array.isArray(allowComponents) &&
            isComponentAllowed(node, allowComponents)
          ) {
            return;
          }
...
function isComponentAllowed(node, allowComponents) {
  var componentName = getComponentName(node);
  if (!componentName) {
    return false;
  }
  return allowComponents.indexOf(componentName) !== -1;
}
...
function getComponentName(node) {
  ...
  return getJSXName(node.parent.name);
}
...
function getJSXName(nameNode) {
  ...
  if (nameNode.type === "JSXMemberExpression") {
    var objectName = getJSXName(nameNode.object);
    var propertyName = getJSXName(nameNode.property);
  ...
  if (nameNode.type === "JSXNamespacedName") {
    if (nameNode.namespace && nameNode.name) {
